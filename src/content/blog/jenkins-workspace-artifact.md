...
第三層是根本解:**每次 build 開一個全新的容器或 pod**(第 11 篇),那就沒有「上一次」可言了。
...
不過老實說,fingerprint 是**補救措施**。更根本的作法是產物本身就帶不可變的版本(語意化版本 + commit SHA),而且 [[sre-automation-release|一顆產物走完所有環境]],不要每個環境各 build 一次——那是第 10 篇的主題。
