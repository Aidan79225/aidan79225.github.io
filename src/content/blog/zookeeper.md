...
<figure style="margin:1.5rem 0;text-align:center;">
  <svg viewBox="0 0 580 210" role="img" aria-label="ZooKeeper 架構:一個 ensemble,寫走 leader,讀就地。左邊 client A 的寫請求一律走 leader,leader 用 Zab 原子廣播給兩個 follower,過半 ack 才 commit。client B 的讀請求由任何一台就地回答,可能稍舊。下方說明:寫是線性化全序、過半才算數;讀可擴展、就地回答可能稍舊、要最新用 sync;另有 observer 不投票專門擴充讀吞吐。" style="width:100%;max-width:600px;height:auto;margin:0 auto;">
    ...
    <text x="290" y="198" fill="#9aa4b2" font-size="8.3" text-anchor="middle">另有 observer(不投票)專門擴充讀吞吐</text>
  </svg>
