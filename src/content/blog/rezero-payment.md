...
三套防禦的成本歸零,因為**沒有可以被打壞的狀態**。這是整個系列「事實 append、狀態派生」主旋律的最終回收:同一個原則,在[[rezero-comment-order|留言層]]教過你事實要接上路徑才算數(raw 留了,重放的路沒建)、在[[rezero-inventory|庫存層]]給了你對帳、在金流層直接讓最兇的一類 bug 絕種。
