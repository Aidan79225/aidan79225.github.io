...
寫 [[airflow-intro|Airflow]] 跟 [[spark-intro|Spark]] 時我都說「別太早上、[[pain-before-power|先確認痛點]]」,因為它們的複雜度與維運成本高。**dbt 剛好相反**:如果你的轉換本來就是 SQL、又跑在資料倉儲上,dbt 的導入成本其實很低(裝起來就能用、學習曲線平),換來的是可測試、有版控、自動產文件與血緣的轉換層。投報率高、下檔風險小,所以這是少數我會說「幾乎可以直接上」的工具。
