...
2. **第一天就用「職能+能力」的雙軌 role。** 職能 role 對齊[[rezero-console|五組介面]]——上一章切介面的判準是「這個人此刻的任務」,權限的正確粒度也是任務:兩者描述的本來就是同一件事,「誰在做什麼」;敏感能力(成本、個資)從第一天就是獨立可疊加的 role——不是預測未來,是承認「正交維度一定會出現」這個規律。
