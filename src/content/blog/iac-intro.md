...
date: 2026-08-05
category: tech
description: "鐵器時代的基礎設施是實體硬體,變更又慢又貴,所以整套管理文化都在防變更;雲時代開機器只是一次 API 呼叫,限制翻轉了,該防的不再是變更,而是不敢變更。Kief Morris 開篇給 IaC 下的定義:把軟體工程的紀律——版本控制、測試、持續交付——整套搬到基礎設施上,讓變更又快又安全。"
tags:
  - iac
  - devops
  - book-notes
...
draft: false
---
開一個新系列:讀 Kief Morris 的 *Infrastructure as Code*。前一篇 [[ansible-intro|Ansible]] 從雪花伺服器講起,講的是「一個工具怎麼解這個問題」;這本書則是把鏡頭拉遠——工具年年在換(CFEngine、Puppet、Ansible、Terraform……),但「用程式碼管基礎設施」背後的原則不會換。第一章回答最根本的兩件事:**IaC 到底是什麼,以及為什麼雲時代的基礎設施非這樣管不可。**
