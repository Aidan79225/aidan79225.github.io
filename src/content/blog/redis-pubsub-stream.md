...
寫到這篇,整個 [[redis-intro|Redis 系列]]剛好收口。回頭看這一路——資料結構、單執行緒、持久化、過期淘汰、快取三災、分散式鎖、複製、Sentinel、Cluster、事務與 Lua、到這篇的訊息系統——每一面其實都在證明開篇那句話:**Redis 是一台記憶體資料結構伺服器,「快取」只是它最出名的一個用法。** 它能當鎖、當佇列、當排行榜、當計數器、當輕量訊息 log,是因為它給了你一套**又快又原子的資料結構**,剩下的是你怎麼組合。這也是我最想留給讀完整個系列的你的一句話:**別把 Redis 框在「快取」那個小盒子裡**——當你開始把它當「手邊一台隨叫隨到的資料結構伺服器」,很多原本要架大東西才能解的問題,會突然變得又輕又簡單。
