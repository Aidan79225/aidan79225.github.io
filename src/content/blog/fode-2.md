...
我越來越覺得,五個階段是「入門就看得到」的東西 —— 誰都知道資料要攝取、要轉換、要服務。真正拉開差距的是 undercurrents:有沒有把安全(別 hardcode 帳密,呼應我寫 [[airflow-providers|Connection]] 那篇)、可觀測性、資料品質、血緣這些「看不見的」做進去。一個 demo 跟一個能在半夜不出事的生產系統,差別幾乎全在暗流。**新手炫的是管線跑通,老手守的是暗流。**
