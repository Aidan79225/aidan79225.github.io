...
先澄清一個容易誤會的點:「用整數算」不代表結餘會是整數。多幣別換匯後金額天生帶著角分——6991 JPY × 匯率 0.2 就是 1398.20 TWD——結餘自然也會有小數。整數運算避免的是**浮點誤差**,不是小數;它給的保證是:那些小數都是精確的「分」,**每筆分攤加總恰好等於支出總額、所有人的結餘加總恰好歸零**,一分不多不少。反過來,硬把結餘湊成整數(四捨五入到元)才會做出一本加總不歸零的帳。所以結餘不取整是刻意的:工具只負責把**事實**精確地算到分;轉帳時要不要湊個整數,留給收付錢的人自己決定——**事實歸機器,派生的決定歸人**。三種分攤模式各有一個「加總守恆」的細節:
