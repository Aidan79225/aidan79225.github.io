...
date: 2026-07-17
category: tech
description: "學一個工具和把它放進生產,是兩種完全不同的問題。學的時候問『怎麼用』;上生產問『它會怎麼掛、怎麼長大、半夜壞了怎麼救、要幾台、資料放哪』。這篇立下一套 Infra 體檢表——部署拓撲、狀態與儲存、擴展、HA、容量、監控、調校、故障模式,套在任何工具上都適用;以及貫穿整個系列的核心軸線:stateful ↔ stateless。一個工具有狀態還是無狀態,幾乎決定了它怎麼擴、怎麼做 HA、在 k8s 上怎麼跑、半夜掛了好不好救。"
tags:
  - infrastructure
  - concept
...
draft: false
---
學一個工具、和把它**放進生產**,是兩種完全不同的問題。學的時候你問「這個 API 怎麼用、這個概念是什麼」;上生產時你問的是另一組問題——**它會怎麼掛?怎麼長大?半夜壞了怎麼救?要幾台、多少記憶體?資料放哪、掉了能不能救回來?** 這個系列,就是用**同一套框架**,把每個工具當成「一塊 infra」來體檢。這第一篇,先把框架立起來。
...
為什麼「狀態」是樞紐?因為一個工具**有狀態(stateful)還是無狀態(stateless)**,幾乎決定了它在生產的一切。這條軸是整個系列最重要的一把尺:
...
我職涯的一個轉捩點,是意識到「會用一個工具」和「能把它養在生產」是兩種能力。前者靠讀文件、跑 tutorial 就會;後者要你回答一堆文件不太教的問題——它半夜三點會怎麼壞、擴容時會不會掉資料、監控該盯什麼才能提早發現。剛工作時我以為「學會用」就夠了,直到第一次被 on-call 叫醒、對著一個「明明教學都跑得好好的」系統束手無策,才懂那之間差了一整個維度。這個系列就是想補上那個維度:**不是教你怎麼用這些工具,而是教你怎麼把它們當 infra 養活、養穩、養大。**
