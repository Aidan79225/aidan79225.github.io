...
這是我想了很久之後的誠實答案:**這五個元件,重來一次我全部保留。** 最大的理由是 Django admin——它的地位幾乎無可取代。營運要查訂單、改庫存、封鎖使用者,這些後台功能用 admin 幾乎是白嫖,註冊個 model 就有;對一個只有 3 個後端的團隊,等於省下一整個後台工程師。第二個理由是 Celery:配上 RabbitMQ 和 heartbeat 排程,你就有了**一套類似 [[infra-airflow|Airflow]] 的能力,但完全不用管 Airflow 的維運**。抓留言、批次下單、發票、對帳 job 全跑在上面。工作流平台是好東西,但它有自己的伺服器要養、自己的坑要踩——在每天十幾個 feature 的節奏裡,「不用管維運」本身就是最大的 feature。
