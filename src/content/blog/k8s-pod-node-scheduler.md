...
這代表:**你永遠不該記住某個 Pod 的 IP、也別把狀態存在 Pod 裡。** Pod 是牛,不是寵物——壞了就換一頭,不是抱著醫。這帶出兩個之後的主題:既然 Pod 的 IP 會變,別的服務怎麼穩定找到它?(→ Service,[[k8s-intro|系列]]後面第 4 篇);Pod 沒了資料就沒了,那資料庫這種有狀態的怎麼辦?(→ Volume 與 StatefulSet,第 6 篇)。
