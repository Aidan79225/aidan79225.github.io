...
date: 2026-08-02
category: tech
description: "終章。政治因素一筆帶過——我們是工程師,專注在該專注的地方;真正要寫的是最後一個月:1 on 1、陪跑面試、一起研究 AI 工具、解散前夕還在交付;一顆隕石的翻案、三個寫作中段才浮現的體悟、「重來也不換」與「重來要補」的兩張清單、AI 浪潮下什麼跟著人走——以及 Re: 這個字,到底是什麼意思。"
tags:
  - war-story
  - live-commerce
  - retrospective
...
draft: false
---
這個系列從[[rezero-overview|一則留言的旅程]]開始,寫了二十章。終章不新增任何架構,只做五件事:交代結局、講完最後一個月、把三個寫到中段才浮現的體悟收起來、回答書名的問題——**如果真的重來,我會改什麼**——最後,在 AI 浪潮裡看一眼:什麼跟著人走。
...
**其三:整個系統,是一台拆開的資料庫。**這條在 [[rezero-reconciliation|#16]] 展開過,終章只補最後一層:前兩個體悟其實是它的推論。「當年做得不錯」,是因為我們無意間遵守了資料庫的內部紀律(先寫日誌、派生不物化、修復照事實);「做了 DE 的事」,是因為拆開的資料庫的每個元件,本來就是 DE 的日常。三個體悟是同一件事的三面——**我們花一年半,親手造了一台資料庫,而且直到寫這個系列,我才知道。**
...
最後一個月我們一起研究 AI 工具,當時是為了求職加分;幾個月後回頭看,那其實是在看清一件更大的事:**AI 把「寫」變便宜了,把「判斷」變貴了。**這個系列裡最值錢的東西,沒有一樣是「寫」出來的——不做 cache 的堅持、拔掉狀態機的認輸、罰其所犯的顆粒度、boring tech 的紀律,全部是判斷,而判斷來自在場。AI 可以給你五種扣庫存的方案,它給不出「這個團隊、這個現場、這個主播,該選哪一個」——那個答案不在訓練資料裡,在直播的聲音裡。

multica 那個最後的交付,現在看是一次預演:非工程師寫 task、工程師 review——把產出讓出去,把把關留下來。[[rezero-team|#18]] 說 lead 的工作是三個把關點:拆解、review、選型;AI 時代,這三件事從 lead 的專利變成**每個工程師的日常**——你跟 AI 的關係,就是 player-coach 跟團隊的關係。所以提升自己的方向很具體:把關能力長在判斷上,判斷長在在場上;而在場的經驗要變成可遷移的資產,只有一條路——**寫下來**。這個系列自己就是示範:三個體悟沒有一個是當年知道的,全是寫作逼出來的。經驗不寫,只是回憶;寫了,才是判斷。

工具每兩年換一輪,追工具的人每兩年歸零;問題那一組——不變量、事實與派生、邊界、取捨——十年沒變過。**握著問題的人,把 AI 當槓桿;只握工具的人,被 AI 當替代。**
