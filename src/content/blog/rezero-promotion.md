...
**第一層:單一商品、單一價格——這是無界背包,不是 NP-hard。** 把問題寫出來:客人買 $n$ 件,優惠「買 $a_i$ 送 $b_i$」可重複套用、每件商品只能被一個優惠算一次,目標是最大化免費件數 $\sum b_i x_i$,限制 $\sum (a_i+b_i)\,x_i \le n$。這是無界背包(找零錢問題的親戚):形式上 weakly NP-hard,但有 $O(n \times k)$ 的 DP——$n$ 是購買件數,現實裡是幾十,瞬間解完。有趣的是,**greedy 在這個最簡單的情境就已經不是最優**。拿「買 3 送 2」(一組吃 5 件、免 2)和「買 5 送 3」(一組吃 8 件、免 3)來算:

- $n=8$:greedy 先套買 5 送 3——免 3 件;最優也是免 3 件。打平。
- $n=10$:greedy 先套買 5 送 3,剩 2 件套不進任何優惠——免 3 件;**最優是買 3 送 2 套兩次——免 4 件。**

greedy 輸的原因跟找零錢一樣:**免費比例高的組合不一定好,尾數才是關鍵**。所以精確地說,當年的 greedy 從來不是「近似最優」,它就是「規則簡單」——而這正是它被選中的理由,不必替它冠上最優的名。

**第二層:同一件商品有三種價格——「最優」的定義開始漂。** 商品有直播價、商城價,cart item 上還有一個「特殊價」欄位給特殊優惠用——同一台購物車裡,同一件商品可能三種價格並存。優惠套下去,免的是哪一件?扣除額按哪個價算?當年**沒有明確定義「哪個價一定最便宜」**——此時最優解演算法最脆弱的地方不是算力,是**規格**:連「最優」都沒有唯一定義,演算法是在最佳化一個沒有人簽名的目標函數。算出來的「最優」,換一種價格解讀就變了。
