...
我不會無腦對所有專案套三層。小專案、資料源單純、只有一兩個報表的情況,硬切三層只是徒增 pipeline 複雜度與運算成本 —— 兩層(原始 + 報表)往往就夠。Medallion 的三層是「當你有多來源、多消費者、需要稽核與重建」時才回本的投資。先確認痛點再分層,別把它當成預設動作 —— 這跟我對 [[airflow-intro|Airflow]]、[[spark-intro|Spark]] 的態度一致:架構是用來解決具體痛點的,不是拿來證明自己很 enterprise 的。
