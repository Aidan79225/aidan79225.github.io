...
然後是全面的 `tenant_id` 巡禮:檔期、商品、購物車、訂單、金流事實表、配貨紀錄、image_metadata——**每一張表都要帶**。巡完會發現一個此前沒人注意的事實:**這個系統的業務天然是租戶局部的**。所有交易都發生在「一個商家與他的客人」之間——留言下單在商家的直播裡、[[rezero-cart-order|跨檔期合併結帳]]是同一個商家的檔期、[[rezero-inventory|結束檔期]]清的是自己的購物車。跨商家的業務動線,一條都沒有。這個觀察是後面每一節的地基。
