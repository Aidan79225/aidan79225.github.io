...
一個很常見的模糊地帶是「手動測試算不算 toil?」——答案是**看哪一種,而分界正好落在「無長期價值」這條**。每次 release 都手動點一遍**相同**的 regression 流程,六個特徵全中,是教科書級的 toil,該把它自動化掉;但**探索性測試**(人憑經驗到處戳、找出新的邊界情況)、第一次測一個全新功能、UX 測試,這些需要人的判斷、每次都在探索新東西、也很難自動化——它們有累積,不是 toil。一句判準:**「這次手動測完,下次還要一模一樣再測一遍嗎?」** 會,就是 toil;不會、每次都在用判斷探索,就是有價值的人力。
