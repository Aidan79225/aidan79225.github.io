# Day 11|第三方金流:教科書的三個坑

交易主線走完,開始收錢。金流是這個系統第一次把「正確」交到別人手上——付款發生在銀行那邊,你只能透過不可靠的網路得知結果。

## 當年的付款版圖

付款方式四種:**兩家銀行**,提供**智慧轉帳**和**信用卡**,外加一個自訂的**現金付款**(營運線下收款後入帳)。而且有個一開始就存在的硬需求:**部分付款**——轉帳單筆上限 30,000,一筆 150,000 的結帳,客人要拆五張轉帳付完。

這個需求宣判了「`is_paid` 布林」的死刑:錢是**分批、分渠道、非同步**到的。當年的資料模型:
...
而當年的系統,這三個坑的體感是:「**好像沒遇到什麼問題**」。不是運氣好——是上面那三個設計決定,讓這三個坑根本沒有立足點。明天把這件事攤開講:為什麼「事實 append、狀態派生」的地形上,教科書的坑一個都挖不出來。
