...
兇手是一行邊界:batch 200 裡只要有**一筆留言 parsing 意外失敗,整批放棄**——一則怪留言,拖著 199 個無辜的訂單陪葬。串流處理的世界管這叫 **poison pill(毒藥訊息)**,而我們的版本特別痛在**爆炸半徑**:失敗的粒度是「批」,不是「筆」。症狀還極具欺騙性:抓留言一切正常、留言瀑布照流,只有轉換率悄悄歸零——監控上最難抓的,就是這種「什麼都沒壞,只是沒有產出」的失敗。

修法是那個 Day 3 講過的「下策」:**單筆失敗直接略過、其他照跑**。放進演化史看:整批死(事故)→ 單筆略過(最快的止血)→ dead-letter 補救(重來版的第三步)。**爆炸半徑先縮小,完整性之後再補**——順序是對的,只是第三步當年沒走完。

## 讀路徑戰記:從 fat API 到雲端菩薩

同步側的戰記是一條完整的因果鏈,按時間順序:
...
1. **列表 API 站穩立場。**列表只回部分需要的欄位,全站一致、堅守 RESTful。這一條要在需求會議裡贏,不是在機房裡贏——前端多要一個欄位很便宜,13,000 人各拿一份就是容量事故。
2. **第一天就把 domain 放上 Cloudflare。**專業的東西交給專業的。
3. **毒藥的正解走完第三步:dead-letter。**爆炸半徑縮到單筆,完整性由補救路徑接住。
4. **給批次積壓裝一個儀表。**「淤而不倒」的前提是**你看得到淤**——batch lag 是這個架構唯一重要的健康指標,它應該在主播開播前就出現在某個螢幕上,而不是等留言區有人問「怎麼還沒收到」。

最後一條反思,是這場 DDoS 教的:**帳單也是攻擊面。**系統動不了是一種傷,帳單多一千美元是另一種——而且你為了防禦加購的服務,也在跟著流量計費。攻擊者的成本趨近零,你的防禦若隨請求計價,這場仗在經濟上就輸了。把攻擊擋在計費表之外(邊緣層、固定費率、免費層),跟把攻擊擋在系統之外一樣重要——**安全架構的一半,是財務架構。**
