...
| # | slug | 標題(暫定) | 主題 | 狀態 |
|---|---|---|---|---|
| 1 | `sql-execution-order` | 你寫的 SQL 不是照你寫的順序跑 | `FROM→WHERE→GROUP BY→HAVING→SELECT→ORDER BY→LIMIT`;為何 WHERE 不能用 SELECT 別名、GROUP BY 在 SELECT 之前 | ⬜ 脊椎・先寫 |
| 2 | `sql-joins` | JOIN 的真相:笛卡爾積再過濾 | INNER/LEFT/FULL/CROSS 的列配對格子圖;`LEFT JOIN` 又在 `WHERE` 篩右表=偷偷變 INNER | ⬜ |
| 3 | `sql-null` | NULL 不是值,是「不知道」 | 三值邏輯 TRUE/FALSE/UNKNOWN;`=NULL` 永遠不成立、`NOT IN (含NULL)` 回空、`COUNT(col)` 跳過 NULL | ⬜ |
| 4 | `sql-group-by` | GROUP BY 到底把資料收成什麼 | rows→groups 收合;`HAVING` vs `WHERE`;為何不能選沒 group 的欄;`GROUPING SETS`/`CUBE` 帶過 | ⬜ |
